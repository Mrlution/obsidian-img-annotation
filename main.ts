...
interface HarmonyOSTabletCanvasMouseScrollPatchSettings {
	mySetting: string;
	zoomSpeedSetting:number;
}

const DEFAULT_SETTINGS: HarmonyOSTabletCanvasMouseScrollPatchSettings = {
	mySetting: 'default',
	zoomSpeedSetting:0.2,
}

export default class HarmonyOSTabletCanvasMouseScrollPatch extends Plugin {
	settings: HarmonyOSTabletCanvasMouseScrollPatchSettings;
...
class SampleSettingTab extends PluginSettingTab {
	plugin: HarmonyOSTabletCanvasMouseScrollPatch;

	constructor(app: App, plugin: HarmonyOSTabletCanvasMouseScrollPatch) {
		super(app, plugin);
		this.plugin = plugin;
	}
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.addText((text) => {
				...
				// 	if(!isNaN(float_value) &&(float_value>=0 && float_value<=1.0)){
				// 		this.plugin.settings.zoomSpeedSetting = float_value;
				// 	}else{
				// 		this.plugin.settings.zoomSpeedSetting = 0.2;
				// 	}
